...
export class CloudOrdersService {
  ...
  async syncUpdateOrderState(
    id: SyncOrderId,
    state: SyncOrderState,
  ): Promise<void> {
    console.log(
      "[AggregateO] Ricevuto segnale di updateState per l'ordine",
      id.getId(),
      'a',
      state,
    );
  ...
  async syncCreateSellOrder(order: SyncSellOrder): Promise<void> {
    ...
    console.log(
      '[Aggregate] Creato SellOrder:',
      JSON.stringify(orderWithUniqueId, null, 2),
    );
  ...
  async syncCreateInternalOrder(order: SyncInternalOrder): Promise<void> {
    ...
    console.log(
      '[Aggregate] Creato InternalOrder:',
      JSON.stringify(orderWithUniqueId, null, 2),
    );

    this.telemetryService.setInsertedOrders(1, order.getWarehouseDeparture());

    await this.cloudOrdersRepositoryMongo.syncAddInternalOrder(
      orderWithUniqueId,
    );
  }
  ...
  async syncCancelOrder(id: SyncOrderId): Promise<void> {
    ...
    if (result) {
      // Se true, è stato aggiornato
      console.log("[Aggregate] Cancellato l'ordine ", id);
    } else
      console.error(
        "[Aggregate] Errore: l'ordine ",
        id,
        ' è già in stato CANCELED.',
      );
  }

  async syncUpdateReservedStock(
    id: SyncOrderId,
    items: SyncOrderItem[],
  ): Promise<void> {
